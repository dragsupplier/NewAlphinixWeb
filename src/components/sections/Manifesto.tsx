...
export function Manifesto() {
  return (
    <section className="relative border-t border-[var(--color-ink)]/10 py-24 md:py-32 overflow-hidden">
      {/* Massive background numeral — composition mark */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-10 right-[-4%] select-none font-display font-black leading-none tracking-[-0.04em] text-[var(--color-ink)]/[0.04]"
        style={{ fontSize: 'clamp(18rem, 36vw, 44rem)' }}
      >
        WHY
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-y-10 lg:gap-x-12">
          <div className="lg:col-span-4">
            ...
            <div className="mt-6 flex items-center gap-3">
              <span className="font-mono text-[0.6875rem] text-[var(--color-muted)]">OBS / I — III</span>
          ...
          <div className="lg:col-span-8 lg:pl-8 lg:border-l lg:border-[var(--color-ink)]/10">
            <ol className="space-y-12 md:space-y-14">
              {observations.map((o, i) => (
                <Reveal as="li" key={o.index} delay={i * 120} className="grid grid-cols-12 gap-x-6">
                  <div className="col-span-12 sm:col-span-2 lg:col-span-1">
                    <span className="inline-flex h-8 items-center justify-center rounded-full bg-[var(--color-accent-500)]/10 px-2.5 font-mono text-[0.75rem] font-bold uppercase tracking-[0.18em] text-[var(--color-accent-600)]">
                      {o.index}
                    </span>
                  </div>
            ...
            <div className="mt-16 relative">
              <div className="absolute left-0 top-0 h-full w-[3px] bg-[var(--color-accent-500)]" />
              <div className="pl-6 border-t border-[var(--color-ink)]/15 pt-8">
                <p className="font-mono text-[0.6875rem] uppercase tracking-[0.18em] text-[var(--color-accent-600)]">
                  Conclusion
                </p>
                <p
                  className="mt-3 font-display font-bold tracking-[-0.02em] text-[var(--color-ink)] text-balance"
                  style={{ fontSize: 'clamp(1.5rem, 2.6vw, 2.125rem)', lineHeight: 1.1 }}
                >
                  Alphinix exists to address all three from one practice — so a problem in any segment can be
                  solved without changing partners.
                </p>
              </div>
            </div>
          </div>
